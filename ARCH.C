...
*/
 ...
 {
  case evt_NCNT_on:
    printf(" Старт операции без учета.\r\n");
    goto m_val1;

  case evt_NCNT_off:
    printf(" Завершена операция без учета.\r\n");
    goto m_val1;
  ...
  case evt_start_f:
    printf(" Старт %sа жидкой фазы",rcv_m[f_rcv]);
m_val0:
...
m_val1:
    printf("Давл.%7.3f МПа;" ,a_str->u.fill.UnitPrice);
    printf("Напряж.АКБ %6.2f V;\r\n" ,a_str->u.fill.Amount);
...
*/
  ...
  case evt_cont_f:
    printf(" Продолжение %sа жидкой фазы",rcv_m[f_rcv]);
    goto m_val;
  case evt_end_gas:
...
m_val:
    printf("\r\nМасса %sа %8.3f кг; " ,rcv_m[f_rcv],a_str->u.fill.VolT    );  //%8.3f
    goto m_val1;
/*
    printf("Плотн.%7.3f кг/м3;" ,a_str->u.fill.Density);
    printf("Темп.%6.2fC;" ,a_str->u.fill.Temp);
...
*/
  ...
  case evt_DLT_off:
    printf(" %s  прерван:Нет питания 380V.",Rcv_m[f_rcv]);
    goto m_val;
  case evt_VLT_off:
    printf(" %s  прерван:АКБ разряжена.",Rcv_m[f_rcv]);
    goto m_val;

  case evt_VLT:
    printf(" АКБ разряжена.\r\n");
    goto m_val1;
 ...
 mm_0:
  ...
  if(itmp)
  {
    printf(" Ошибка:");
 mm_e:
    printf("%01X" ,a_str->u.err.err[0] );     // WDOG  MMI
    printf(" %01X" ,a_str->u.err.err[1] );    // MMI  MVD
    printf(" %04X" ,a_str->u.err.err[2] );    // MVD1
    printf(" %04X" ,a_str->u.err.err[3] );    // MVD2
    printf(" %01X" ,a_str->u.err.err[4] );    // I7060[0]
    printf(" %01X" ,a_str->u.err.err[5] );    // I7017C[0]
    printf(" %04X" ,a_str->u.err.err[6] );    // System
...
// switch ((int)evt)
 ...
 s_arch->MassI  = o.f;
   ...
   case evt_verify :
   case evt_verify_e :
    s_arch->u.fill.VolT       = dose_A;
    s_arch->u.fill.Amount     = s_MVD[0].Volt_A;
   ...
   case evt_VLT:       // АКБ разряжена
   case evt_VLT_off:   // АКБ разряжена, прием/отпуск завершен
   case evt_eq_pr  :   // Выравнивание давлений газовой фазы
   case evt_wd_pmp :   // Таймаут связи
   if(flag_rcv != 0)
      s_arch->u.fill.VolT=s_MVD[0].MassT + s_MVD[1].MassT;
   else
      s_arch->u.fill.VolT=-(s_MVD[0].MassT + s_MVD[1].MassT);
 ...
 s_arch->MassI  = s_MVD[0].MassI+s_MVD[1].MassI;
...
//  if(ltim==ltim_1) return -1;
 ...
 if((ar_str_ptr->MassI==ar_str_ptr_1->MassI) && (ar_str_ptr->MassI != vBIG_P))
 {
   tMassI=ar_str_ptr->MassI;
   tVolI =ar_str_ptr->VolI;
 //printf("\r\n 2");
