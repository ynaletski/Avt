...
int Delta_SV_beg[max_Dlt]={5000,5000};  // 50 Hz, значение частоты вращения
...
int Delta_SA_beg[max_Dlt]={20,20}; // текущее состояние времени ускорения 2 сек
int Delta_SA[max_Dlt]={20,20};     // текущее состояние  ускорения 2 сек
int Delta_SAc[max_Dlt]={-1,-1};    // текущее состояние  ускорения

int Delta_SD_beg[max_Dlt]={20,20};  // текущее состояние  замедления 2 сек на 100Гц
...
//     f_Dlt_wd(ii);
 ...
 if(Delta_Ctrl[ii] != Delta_Ctrl_c[ii] )
 {
      fn=Dlt_CTRL_OUT;
      goto m0;
 }
 else if(Delta_fn[ii] != 0)
      fn=Delta_fn[ii];
...
// if( f_timer(Delta[ii].time_stamp_pool,Delta[ii].pool_time ));
// else return;
...
if(DLT_DEB != 0)
  printf("\n\r TOUT ,fn=%d,time=%ld",fn,TimeStamp);
...
m_err:
 ...
 switch (fn)
 {
  case Dlt_SV_OUT  :
if(DLT_DEB != 0)
  printf("\n DLT_SV_OUT = %d,time=%ld",Delta_SVc[nn],TimeStamp);
  ...
  case Dlt_SA_OUT  :
if(DLT_DEB != 0)
 printf("\n DLT_SA_OUT = %d,time=%ld",Delta_SAc[nn],TimeStamp);
       Delta_SAc[nn]=-1;
       break;
  case Dlt_SD_OUT  :

 if(DLT_DEB != 0)
 printf("\n DLT_SD_OUT = %d,time=%ld",Delta_SDc[nn],TimeStamp);
 ...
 if(DLT_DEB != 0)
   printf("\n DLT_CTRL_OUT = %4x,time=%ld",Delta_Ctrl_c[nn],TimeStamp);
       Delta_Ctrl_c[nn]=-1;
       break;
  case Dlt_RES_ERR  :
if(DLT_DEB != 0)
  printf("\n DLT_RES_ERR ,time=%ld",TimeStamp);
       Delta_fn[nn]=fn;
       break;
  case Dlt_SET_MODE :
if(DLT_DEB != 0)
  printf("\n DLT_SET_MODE,time=%ld",TimeStamp);
       Delta_fn[nn]=fn;
       break;
  case Dlt_SET_CTR :
if(DLT_DEB != 0)
  printf("\n DLT_SET_CTR ,time=%ld",TimeStamp);
       Delta_fn[nn]=fn;
       break;
 }
 Delta_inp_err[nn]=0;
