...
{
   // Регулятор перепада.
   // Формирует заданное значение P_dem_c для регулятора давления, исходя из
   // условия незакипания жидкости
...
if(flag_flow == 0)
 if( fabs(s_MVD[0].FlowM) >= Flow_on[stage_c>>1] ) flag_flow=1;
else
 if( fabs(s_MVD[0].FlowM) < Flow_off[stage_c>>1] ) flag_flow=0;

if(flag_flow)
 {
   XdN = (long int) (( P_real - s_MVD[0].PressG - s_MVD[0].PressB - P_Dif_d) * K_F_INT );
